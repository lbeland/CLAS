...
| `compareALGO.py` | Bland–Altman / scatter of the `fooof` vs `myIAF` IAF estimators over `results/` |
| `eeg/helpers_ext.py` | per-window IAF + `f0_seq` tracking, `load_ds004148`, `time_s` in the IAF CSV |
| `eeg/simple_iaf.py` | `combine_simple` (+ helpers) copied verbatim from `IAF_Estimation/IAF_tests.py`; `simple_paf` adapter |
| `eeg/run_pipeline.py` | driver: adds `ds004148`, `--iaf-method {fooof,simple}`, progress bar, CPU affinity, `--subjects` |
| `eeg/plot_results.py` | driver: `compute_iaf_change_rate`, ds004148 participant-id regex, extra CSV dumps |
...
```bash
# EEG phase-error pipeline (ds004148, channel Fz, fooof estimator).
# Outputs go to cecHT_ext/results/<dataset>_<channel>_<method>/  (here: results/ds004148_Fz_fooof/).
...
#   ^ figure + iaf_cv / iaf_change_rate CSVs, written into that run dir
