...
def phase_reconst(data, tau, max_cut_len=None):
    ...
    -------
        ...
        Phase
        phase_h: ndarray with size of (1, N)
        Hilbert-transform phase (before FFT-based reconstruction), same cut as phase_new.
        initindx: int
        Number of samples cut from the start of `data` so that phase_h[0][0] is ~zero.
            Callers that need to align the output back to `data`'s original indexing
            (e.g. to compare against a ground-truth phase of the same length as `data`)
            should offset by this amount.
    """
    ...
    return phase_new, phase_h, initindx
