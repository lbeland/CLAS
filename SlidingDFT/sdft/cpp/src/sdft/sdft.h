...
namespace sdft
{
  ...
  public:
      ...
      dftsize(dftsize)
    {
      ...
      analysis.fiddles.resize(dftsize, std::complex<F>(1, 0));

      const F pi = std::acos(F(-1));
      const size_t N = dftsize;
      const size_t L = 2 * N;

      const F omega = F(-2) * pi / F(L);

      for (size_t i = 0; i < N; ++i)
      {
        // analysis stays unchanged
        analysis.twiddles[i] = std::polar(F(1), omega * F(i));

        // synthesis now reconstructs the last sample
        const F phase_last = F(2) * pi * F(i) * F(L - 1) / F(L);
        synthesis.twiddles[i] = std::polar(F(1), phase_last);
      }
    }
