...
def ampl_mod(t, A_c, f_c, A_m, f_m):
    ...
    amplitude = A_c + A_m * math.cos(w_m * t)
    theta = w_c * t
...
def EEG_simulate(n_samples=-1):
    ...
    Modulation_type = "phase"   # amplitude, phase or None
        ...
        while True:
            ...
            if Modulation_type != "None":
                if Modulation_type == "amplitude": 
                    value, amplitude, theta, inst_freq = ampl_mod(t, carrier_signal["amplitude"], carrier_signal["frequency"], mod_signal["amplitude"], mod_signal["frequency"])

                elif Modulation_type == "phase":
                    value, amplitude, theta, inst_freq = angle_mod(t, carrier_signal["amplitude"], carrier_signal["frequency"], mod_signal["amplitude"], mod_signal["frequency"])

            else:
                value = carrier_signal["amplitude"] * math.cos(2.0 * math.pi * carrier_signal["frequency"] * t)
                amplitude = carrier_signal["amplitude"]
